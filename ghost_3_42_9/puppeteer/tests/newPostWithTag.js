...
async function testNewPostWithTag() {
  ...
  const { page, logStep, screenshotsDir, browser } =
    await initialSettings("newPostWithTag");
  ...
  try {
    ...
    // And
    await postPage.assignTags();
    await logStep(
      "Asignar tags",
      path.join(screenshotsDir, "06_assignTags.png")
    );
    // And
    await postPage.publishPost();
    await logStep(
      "Publicación de post",
      path.join(screenshotsDir, "07_publishPost.png")
    );
    ...
    await logStep(
      ...
      path.join(screenshotsDir, "08_finalReview.png")
    );
    ...
    await logStep(
      ...
      path.join(screenshotsDir, "09_confirmPublish.png")
    );
    ...
    await logStep(
      ...
      path.join(screenshotsDir, "10_newPostSuccess.png")
    );

    await postPage.returnToPosts();
    await logStep(
      "Retornar a pagina de posts",
      path.join(screenshotsDir, "11_returnToPosts.png")
    );
    ...
    if (postName === title) {
      await logStep(
        ...
        path.join(screenshotsDir, "12_newpostSuccess.png")
      );
    } else {
      await logStep(
        "No se creó el post",
        path.join(screenshotsDir, "12_newpostError.png")
      );
    }
  } catch (error) {
    await logStep(
    ...
    await logStep(
      "Se cierra sesión",
      path.join(screenshotsDir, "13_logOut.png")
    );
    await browser.close();
  }
}
