...
async function testNewPremiumPost() {
  console.log("REALIZANDO PRUEBA E2E DE CREAR UN POST PREMIUM");
  const url = "https://ghost-5ehz.onrender.com/ghost/#/signin";
  const { page, logStep, screenshotsDir, browser } =
    await initialSettings("newPremiumPost");
  ...
  try {
    ...
    //And
    await postPage.convertToPaid();
    await logStep(
      "Convertir post a pago",
      path.join(screenshotsDir, "06_convertToPaid.png")
    );
    ...
    if (postName === title) {
      await logStep(
        ...
        path.join(screenshotsDir, "11_newpostSuccess.png")
      );
    } else {
      await logStep(
        "No se creó el post",
        path.join(screenshotsDir, "11_newpostError.png")
      );
    }
  } catch (error) {
    await logStep(
...
module.exports = testNewPremiumPost;
